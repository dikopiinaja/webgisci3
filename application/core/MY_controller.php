...
class MY_Controller extends CI_Controller{
    ...
    public function authenticated(){ // Fungsi ini berguna untuk mengecek apakah user sudah login atau belum
        ...
        if($this->uri->segment(1) != 'auth' && $this->uri->segment(1) != ''){
            ...
            $this->session->set_flashdata('message', 'swal("Gagal!", "Silahkan Login terlebih dahulu", "warning");');
                redirect('user/auth'); // Redirect ke halaman login
        }
    }
    ...
    public function render_backend($content, $data = NULL){
        ...
        * */
        // $data['template'] = $this->load->view('user/_partials/index', $data, TRUE);
        $data['content'] = $this->load->view($content, $data, TRUE);
        $this->load->view('user/_partials/index', $data);
    }
}
